...
{
  int maxSizeOne = 12 + (blocksize(sb) / 4);
  int maxSizeTwo = maxSizeOne + ((blocksize(sb) / 4) * (blocksize(sb) / 4));
  int maxSizeThree = maxSizeTwo + ((blocksize(sb) / 4) * (blocksize(sb) / 4) 
				   * (blocksize(sb) / 4));
  ...
  //Indirect Block
  else if (n < maxSizeOne) {
    get_block_data(sb, i->i_block_1i, buffOne);
    byteswap_iblock(sb, buffOne);
    get_block_data(sb, buffOne[n-11], out);
    }
  //Doubly Indirect Block
  else if (n < maxSizeTwo) {
    get_block_data(sb, i->i_block_2i, buffOne);
   byteswap_iblock(sb, buffOne);
    get_block_data(sb, buffOne[n-(maxSizeOne+pointersPerBlock)/pointersPerBlock], buffTwo);
    byteswap_iblock(sb, buffTwo);
    get_block_data(sb, buffTwo[n-(maxSizeOne+pointersPerBlock)%pointersPerBlock], out);   
  }
  //Triply Indirect Block
  else if (n < maxSizeThree) {
    get_block_data(sb, i->i_block_3i, buffOne);
    byteswap_iblock(sb, buffOne);
    get_block_data(sb, buffOne[n-(maxSizeTwo+pointersPerBlock)/pointersPerBlock], buffTwo);
    byteswap_iblock(sb, buffTwo);
    get_block_data(sb, buffTwo[n-(maxSizeTwo+pointersPerBlock)/pointersPerBlock], buffThree);
    byteswap_iblock(sb, buffThree);
    get_block_data(sb, buffThree[n-(maxSizeTwo+pointersPerBlock)%pointersPerBlock], out);   
  }
  byteswap_iblock(sb, out);
  return 0;
}
...
int is_block_free(struct superblock *sb, int blk)
{
  ...
  get_bgdesc(sb, bg, &temp);
  uint32_t bgBlockBitmap = temp.bg_block_bitmap;
  int indexBit = blk - temp.bg_block_bitmap; //index of the bit in the bitmap
  char ans = bgBlockBitmap - (indexBit / 8);
  char mask = 1 << (index % 8);
  return (ans & mask) == 0;
}
...
int looks_indirect(struct superblock *sb, char *block)
{
  ...
  if (arr[0] == 0)
    return 0;
  for ( i = 0; i < blocksize(sb); i++) {
    if (arr[i] == zeroFlag)
      zeroFlag = 1;
    if (zeroFlag == 1 && (arr[i] != 0))
      return 0;
    if (arr[i] > sb->s_blocks_per_group)
      return 0;
	}
  return 1;
}
...
int looks_2indirect(struct superblock *sb, char *block)
{
  if (looks_indirect(sb, block) == 1) {
    *block = *block + 1025; // looking for two indirect in a row
    if (looks_indirect(sb, block) == 1)
      return 1;
  }
  return 0;
